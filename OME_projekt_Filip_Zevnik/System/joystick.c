...
typedef struct
{
	...
	uint16_t 	position_raw[2];			// meritev trenutne pozicije osi "joysticka"

	uint16_t 	position_raw_min[2];			// informacija o najmanjšem odklonu osi "joysticka"
	uint16_t 	position_raw_max[2];			// informacija o največjem odklonu osi "joysticka"
	uint16_t 	position_raw_range[2];			// informacija o razponu odklona osi "joysticka" (angl. axis range)
...
void JOY_calibrate(void)
{
	...
	while (pressed_key == JOY_BTN_NONE)
	{
		...
		//pressed_key = JOY_get_button_state();
		JOY_scan_button();
