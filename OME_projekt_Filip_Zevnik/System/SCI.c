...
uint8_t SCI_is_data_waiting(void)
{
	...
	uint8_t rez = LL_USART_IsActiveFlag_RXNE_RXFNE(SCI.enota);
...
void SCI_send_string_IT(char *str)
{
	...
	while( *str )
	{
		BUF_store_byte(&SCI_TX_buf_handle, (uint8_t)(*str));
		str++;
		i++;
	}
	...
	if ( i > 0 )
	{
		...
		if ( LL_USART_IsActiveFlag_TXE_TXFNF(SCI.enota) )		// TX empty?
		{
...
void SCI_send_bytes_IT(uint8_t *data, uint32_t size)
{
	...
	{
		BUF_store_byte( &SCI_TX_buf_handle, *data );
	}
	...
	if ( size > 0 )
	{
		...
		if ( LL_USART_IsActiveFlag_TXE_TXFNF(SCI.enota) )		// TX empty?
		{
			...
			LL_USART_TransmitData8(SCI.enota, d);
