...
void LED_demo(void)
{
	...
	// DOPOLNI
	/*LL_GPIO_SetOutputPin( LED_group.LEDs[LED0].port, LED_group.LEDs[LED0].pin );
	HAL_Delay(500);
	LL_GPIO_ResetOutputPin( LED_group.LEDs[LED0].port, LED_group.LEDs[LED0].pin );
	HAL_Delay(500);*/
	...
	for(int i=0;i<100;i++){
		LEDs_write(maska);
		if(maska == 0x80) mode = 1;
		else if(maska == 0x01) mode = 0;
		if(mode == 0) maska = (maska<<1);
		else maska = (maska >> 1);
		HAL_Delay(75);
	}
