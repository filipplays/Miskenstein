...
static inline void FMC_BANK1_SetAddress (LCD_IO_Data_t address)
{
	*(volatile LCD_IO_Data_t *)(FMC_BANK1_REG) = address;
	...
	// Počakaj na sinhronizacijo pomnilnika (sprazni pipeline)
	__DSB();
	//__ISB(); (to je nekaj tilen predlagal, pa ni bilo razlike)
}
...
{
	return *(volatile LCD_IO_Data_t *)(FMC_BANK1_MEM);
	// Počakaj na sinhronizacijo pomnilnika (sprazni pipeline)
}
 ...
 */
//__attribute__(( optimize("-O3") ))	//smo probaval, če to pomaga in je pomagalo, ampak sedaj ni potrebno več
/*!
...
int32_t ILI9341_SendRepeatedData(LCD_IO_Data_t data, uint32_t num_copies)
{
	...
	if (hLCDDMA.Init.PeriphInc == DMA_PINC_ENABLE)	//za Repated data je potrebno onemogočit peripheral increment
	{
		//Cakaj, ce je slucajno sredi transferja
		while (hLCDDMA.State != HAL_DMA_STATE_READY) {;};
